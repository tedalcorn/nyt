...
def main():
    ...
    for gname, polys in european_polys.items():
        ...
        # Top tag must (a) clear the disproportion threshold and (b) span
        # at least 2 distinct calendar years. The year-span filter drops
        # one-cluster signals (DR's nightclub roof collapse, Paraguay's
        # COVID year, Colombia's 2012 Cartagena summit, etc.).
        tag_years_map = country_res.get('tag_years', {})
        top = None
        for cand in recurring:
            if cand['score'] < MIN_SCORE_TO_LABEL:
                break
            if len(tag_years_map.get(cand['tag'], {})) >= 2:
                top = cand
                break
        if top is None:
            continue
    ...
    out_dir = os.path.join(PROJECT_DIR, 'outputs', '2026-05-top-keyword',
                           '2026-05-13-world-country-tweets', 'Europe')
